    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black">{result.route?.distanceKm || 0} km</div>
          </div>
          <div className="text-right">
            <div className="text-sm text-gray-600">Recommended</div>
            <div className="text-xl font-bold">{result.route?.recommendedMode || 'Cab'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {(result.recommendations || []).map((item) => (
          <TransportCard key={item.mode} mode={item.mode} fare={item.fare} time={item.time} badge={item.isRecommended? 'best' : item.isCheapest? 'cheapest' : item.isFastest? 'fastest' : item.badge || 'default'} note={item.note} />
        ))}
      </div>

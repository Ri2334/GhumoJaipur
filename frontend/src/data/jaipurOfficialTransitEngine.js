// OFFICIAL JCTSL 27 CITY BUS ROUTES & REAL PLACE BUS STOP MAPPING ENGINE
// Parsed directly from official JCTSL Urban & Sub-Urban Bus Network Data

export const OFFICIAL_JCTSL_ROUTES = [
  {
    "routeNo": "AC 1",
...
export function findOfficialJCTSBusRoute(originPlaceName, destPlaceName) {
  ...
  for (const hub of HUBS) {
    for (const r1 of OFFICIAL_JCTSL_ROUTES) {
      const oIdx = r1.stops.findIndex(s => s.toLowerCase() === origStopName.toLowerCase() || origStopName.toLowerCase().includes(s.toLowerCase()));
      const h1Idx = r1.stops.findIndex(s => s.toLowerCase() === hub.toLowerCase() || hub.toLowerCase().includes(s.toLowerCase()));

      if (oIdx !== -1 && h1Idx !== -1 && oIdx !== h1Idx) {
        for (const r2 of OFFICIAL_JCTSL_ROUTES) {
          if (r1.routeNo === r2.routeNo) continue;

          const h2Idx = r2.stops.findIndex(s => s.toLowerCase() === hub.toLowerCase() || hub.toLowerCase().includes(s.toLowerCase()));
          const dIdx = r2.stops.findIndex(s => s.toLowerCase() === destStopName.toLowerCase() || destStopName.toLowerCase().includes(s.toLowerCase()));

          if (h2Idx !== -1 && dIdx !== -1 && h2Idx !== dIdx) {
            const s1 = Math.min(oIdx, h1Idx);
            const e1 = Math.max(oIdx, h1Idx);
            let leg1Stops = r1.stops.slice(s1, e1 + 1);
            if (oIdx > h1Idx) leg1Stops = leg1Stops.reverse();

            const s2 = Math.min(h2Idx, dIdx);
            const e2 = Math.max(h2Idx, dIdx);
            let leg2Stops = r2.stops.slice(s2, e2 + 1);
            if (h2Idx > dIdx) leg2Stops = leg2Stops.reverse();

            return {
              type: 'transfer',
              transfers: 1,
              transferStop: hub,
              busNumber: `Route ${r1.routeNo} ➔ Route ${r2.routeNo}`,
              sourceStop: origStopName,
              destStop: destStopName,
              originPlace: originPlaceName,
              destPlace: destPlaceName,
              ...
              lastLegWalk: destInfo.walkTime,
              fare: 30,
              estimatedTimeMinutes: Math.max(25, (leg1Stops.length + leg2Stops.length) * 3.5),
              waitingTimeMinutes: 10,
              route1: {
                routeNumber: `Route ${r1.routeNo}`,
                routeName: r1.name,
                stopsPassed: leg1Stops
              },
              route2: {
                routeNumber: `Route ${r2.routeNo}`,
                routeName: r2.name,
                stopsPassed: leg2Stops
              }
            };
          }
        }
